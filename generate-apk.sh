...
if [ "$TRAVIS_REPO_SLUG" != "systers/volunteers-android" ]; then
...
if [ "$TRAVIS_BRANCH" == "develop" ]; then
    ...
    cp $HOME/build/systers/volunteers-android/app/build/outputs/apk/app-debug.apk $HOME/apk/
    ...
    git push https://m-murad:$GITHUB_API_KEY@github.com/systers/volunteers-android apk --force --quiet> /dev/null
    if [ $? -eq 0 ]; then
